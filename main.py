...
class AutoYuntai(object):
    def __init__(self, cap, target_shape, red_upper1=None, red_lower1=None,
                 red_lower_black=None, red_upper_black=None,
        ...
        self.red_upper1 = red_upper1 if red_upper1 else np.array(self.datas['red_upper1'])
        self.red_upper_black = red_upper_black if red_upper_black else np.array(self.datas['red_upper_black'])
        self.red_lower1 = red_lower1 if red_lower1 else np.array(self.datas['red_lower1'])
        self.red_lower_black = red_lower_black if red_lower_black else np.array(self.datas['red_lower_black'])
    ...
    @staticmethod
    def cha_zhi(point1, point2, points=10):
        point1_x = point1[0]
        point2_x = point2[0]
        point1_y = point1[1]
        point2_y = point2[1]
    ...
    def trace_red_black_err_point(self):
        """第三问误差获取"""
    ...
    def trace_edge(self, img, *args, **kwargs):
        ...
...
yun_tai = AutoYuntai(2, (399, 399), uart=uart)
yun_tai.set_init()
...
def find_balck_rect(arg):
    while True:
        ...
        rect = rect[::-1]
        p1, p2, p3, p4 = rect
        list0x, list0y = yun_tai.cha_zhi((0, 0), p2)
        ...
        list_rect_x = list0x + list1x + list2x + list3x + list4x
        list_rect_y = list0y + list1y + list2y + list3y + list4y
...
def task1(arg):
    aim_list_x1, aim_list_y1 = yun_tai.cha_zhi((0, 0), (-333 / 2, 333 / 2))
    aim_list_x2, aim_list_y2 = yun_tai.cha_zhi((-333 / 2, 333 / 2), (333 / 2, 333 / 2))
    aim_list_x3, aim_list_y3 = yun_tai.cha_zhi((333 / 2, 333 / 2), (333 / 2, -333 / 2))
    aim_list_x4, aim_list_y4 = yun_tai.cha_zhi((333 / 2, -333 / 2), (-333 / 2, -333 / 2))
    aim_list_x5, aim_list_y5 = yun_tai.cha_zhi((-333 / 2, -333 / 2), (-333 / 2, 333 / 2))
    aim_list_x = aim_list_x1 + aim_list_x2 + aim_list_x3 + aim_list_x4 + aim_list_x5
    aim_list_y = aim_list_y1 + aim_list_y2 + aim_list_y3 + aim_list_y4 + aim_list_y5
    t1 = time.time()
    while True:
        if time.time() - t1 >= 10:
            break
    ...
    while True:
        ...
        print("black值 {} {}  red值 {} {}".format(set_x, set_y, now_x, now_y))
...
if __name__ == '__main__':
    hsv_red_lower = np.array([0, 0, 25])
    hsv_red_upper = np.array([255, 255, 255])

    root = tk.Tk()
    app = UI(root, [task_reset, task1, func2, set_red_hsv, set_red_black_hsv, set_black_hsv, find_balck_rect])
    root.mainloop()
